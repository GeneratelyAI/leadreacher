...
test("content approval preserves the canvas, summary and browser history", async ({ page }) => {
  ...
  const previousBusiness = await business.textContent();
...
test("live campaign pill completes only persisted approvals across the revised flow", async ({ page }) => {
  ...
  await expect(prospects).toHaveAttribute("data-campaign-state", "pending");
  ...
  await expect(prospects).toHaveAttribute("data-campaign-state", "complete");
  await expect(prospects).toContainText("Founder");
  const content = pill.locator("[data-campaign-section-id='content']");
  await expect(content).toHaveAttribute("data-campaign-state", "pending");
  await expect(content.locator("p svg")).toHaveCount(0);
  await page.getByRole("button", { name: "Continue", exact: true }).click();
  await expect(page).toHaveURL(/\/onboarding-preview\/campaign-content\/personalized-video/);
  await expect(content).not.toHaveAttribute("data-campaign-state", "complete");
  await expect(content.locator("p svg")).toHaveCount(0);
  await page.getByRole("button", { name: "Use this", exact: true }).click();
  await expect(page).toHaveURL(/\/onboarding-preview\/cta/);
  await expect(content).toHaveAttribute("data-campaign-state", "complete");
...
test("audience drafts survive introduction, history, and refresh without approval", async ({ page }) => {
  ...
  await expect(page.locator("[data-campaign-section-id='targeting']")).toHaveAttribute("data-campaign-state", "pending");
  await expect(page.locator("[data-campaign-section-id='targeting'] p svg")).toHaveCount(0);
