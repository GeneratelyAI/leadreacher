...
import { Captcha, isCaptchaEnabled } from "@/features/authentication/components/Captcha";
import Layout from "@/features/authentication/public/Layout";
import { PasswordRules } from "@/features/authentication/components/PasswordRules";
import { Button } from "@/components/ui/Button";
import { Card, CardContent } from "@/components/ui/Card";
import { Input } from "@/components/ui/Input";
import { Label } from "@/components/ui/label";
import { createClient, getBrowserSession } from "@/platform/auth/client";
import { authErrorMessage } from "@/features/authentication/public/auth-errors";
import { MIN_PASSWORD_LENGTH, validateNewPassword } from "@/features/authentication/public/password-policy";
      ...
      <Card className="mx-4 w-full max-w-md sm:mx-auto">
        <CardContent className="p-7 sm:p-9">
          ...
          {complete ? (
            ...
            <form onSubmit={submit} className="mt-6 space-y-4">
              <div className="space-y-2"><Label htmlFor="new-password">New password</Label><Input id="new-password" type="password" autoComplete="new-password" minLength={MIN_PASSWORD_LENGTH} required value={password} onChange={(event) => setPassword(event.target.value)} /><PasswordRules password={password} /></div>
