...
const CHANNEL_IMAGES = {
  linkedin: "/dashboard/linkedin-logo.png",
...
function ChannelLogo({ channel, size = "md" }: { channel: keyof typeof CHANNEL_IMAGES | "whatsapp"; size?: "sm" | "md" }) {
  ...
  if (channel === "whatsapp") {
    return (
      <span className={cn("inline-flex items-center justify-center rounded-md bg-[#25D366] p-1 text-white", dimensions)} aria-label="WhatsApp">
...
function ProductShell({ children }: { children: ReactNode }) {
  return (
    <div className="flex h-full min-h-0 flex-col bg-[#fbfbfe]">
      ...
      <div className="grid h-full grid-cols-[0.82fr_1.18fr] gap-3 p-3 sm:gap-6 sm:p-7">
        <div className="flex min-w-0 flex-col justify-center">
          ...
          <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-[#ebe9f4] sm:mt-5">
            <motion.div className="h-full rounded-full bg-[#4e28df]" initial={{ width: "18%" }} animate={{ width: "100%" }} transition={{ duration: 1.4, ease: "easeOut" }} />
          </div>
        ...
        <div className="flex min-w-0 flex-col justify-center gap-2 sm:gap-3">
          {[
            ...
            <motion.div key={label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18 + index * 0.12 }} className="rounded-lg border border-[#e4e1ef] bg-white p-2.5 shadow-[0_5px_20px_rgba(56,43,107,0.05)] sm:rounded-xl sm:p-4">
              <p className="text-[8px] font-semibold uppercase text-[#8a8fa0] sm:text-[10px]">{label}</p>
      ...
      <div className="flex h-full flex-col p-3 sm:p-6">
        ...
        <div className="mt-3 grid flex-1 grid-cols-3 gap-2 sm:mt-6 sm:gap-4">
          {items.map(({ label, value, icon: Icon }, index) => (
            <motion.div key={label} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="flex min-w-0 flex-col justify-center rounded-lg border border-[#e4e1ef] bg-white p-2 sm:rounded-xl sm:p-4">
              <Icon className="size-4 text-[#4e28df] sm:size-6" aria-hidden /><p className="mt-2 text-[8px] text-[#7a8092] sm:text-xs">{label}</p><p className="mt-1 text-[9px] font-semibold leading-tight sm:text-sm">{value}</p>
            </motion.div>
          ))}
        </div>
        <div className="mt-3 flex items-center gap-2 border-t border-[#e4e1ef] pt-3 sm:mt-5 sm:gap-4 sm:pt-4">
          <span className="text-[8px] font-medium text-[#73798b] sm:text-xs">Recommended channels</span>
      ...
      <div className="flex h-full flex-col p-2.5 sm:p-5">
        ...
        <div className="mt-2 overflow-hidden rounded-lg border border-[#e2dfeb] bg-white sm:mt-4 sm:rounded-xl">
          ...
          {PROSPECTS.map(([name, role, company, status], index) => (
            <motion.div key={name} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.08 }} className="grid grid-cols-[1.35fr_1fr_.7fr] items-center border-t border-[#eeecf3] px-2 py-1.5 sm:px-4 sm:py-2.5">
...
function StagePreview({ stageId, reducedMotion }: { stageId: ProductStoryStageId; reducedMotion: boolean }) {
  ...
  return (
    <AnimatePresence mode="wait" initial={false}>
      <motion.div key={stageId} className="h-full" initial={reducedMotion ? { opacity: 0 } : { opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={reducedMotion ? { opacity: 0 } : { opacity: 0, y: -10 }} transition={{ duration: reducedMotion ? 0.1 : 0.3, ease: "easeOut" }}>
        <Preview />
      </motion.div>
    </AnimatePresence>
  );
...
function DashboardStageImage({ stageId, reducedMotion }: { stageId: ProductStoryStageId; reducedMotion: boolean }) {
  ...
  return (
    <AnimatePresence mode="wait" initial={false}>
      <motion.div key={stageId} className="relative h-full bg-white" initial={reducedMotion ? { opacity: 0 } : { opacity: 0, scale: 1.015 }} animate={{ opacity: 1, scale: 1 }} exit={reducedMotion ? { opacity: 0 } : { opacity: 0, scale: 0.992 }} transition={{ duration: reducedMotion ? 0.1 : 0.38, ease: "easeOut" }}>
        <Image
          src={image.src}
          alt={image.alt}
          fill
...
function StageTabs({ activeIndex, onSelect }: { activeIndex: number; onSelect: (index: number) => void }) {
  function handleKeyDown(event: KeyboardEvent<HTMLDivElement>) {
    ...
    onSelect((activeIndex + direction + STAGES.length) % STAGES.length);
  }
  return (
...
function DesktopStory() {
  ...
  return (
    <div data-navbar-theme="dark" className="relative hidden overflow-clip rounded-t-[36px] bg-[#0d1020] text-white md:block lg:rounded-t-[48px]">
      ...
      <ContainerScroll className="relative" id="product-story-scroll" onProgress={handleProgress} reducedMotion={reducedMotion} titleComponent={<div><p className="text-xs font-semibold uppercase text-[#aa96ff]">See LeadReacher in action</p><h2 className="mx-auto mt-3 max-w-3xl text-balance text-4xl font-semibold text-white lg:text-5xl">We handle everything. You close the deal.</h2></div>}>
...
function MobileStory() {
  ...
  return (
    <div data-navbar-theme="dark" className="relative overflow-hidden rounded-t-[28px] bg-[#0d1020] pt-8 text-white md:hidden">
      ...
      <div className="sticky top-16 z-20 mx-4 mb-8 overflow-hidden rounded-xl border border-white/12 bg-[#171a2e]/92 p-1 shadow-lg backdrop-blur-xl"><div className="grid grid-cols-5">{STAGES.map((stage, index) => <a key={stage.id} href={`#mobile-story-${stage.id}`} aria-label={`Show ${stage.label} stage`} className={cn("rounded-lg px-1 py-2 text-center text-[9px] font-semibold text-white/50", index === activeIndex && "bg-[#6842f5] text-white")}>{index + 1}</a>)}</div></div>
