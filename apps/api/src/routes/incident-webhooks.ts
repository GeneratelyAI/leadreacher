...
export async function incidentWebhookRoutes(app: FastifyInstance): Promise<void> {
  app.post("/webhooks/incidents/sentry", { bodyLimit: MAX_WEBHOOK_BYTES }, async (request, reply) => {
    ...
    const nativeSignatureIsValid = verifySentryWebhookSignature(
      request.body,
      request.headers[SENTRY_HOOK_SIGNATURE_HEADER],
      env.SENTRY_WEBHOOK_SECRET,
    );
    const legacySecretIsValid = verifyIncidentWebhookSecret(
      request.headers[INCIDENT_WEBHOOK_SECRET_HEADER],
      env.SENTRY_WEBHOOK_SECRET,
    );
    if (!nativeSignatureIsValid && !legacySecretIsValid) throw new AuthError();
