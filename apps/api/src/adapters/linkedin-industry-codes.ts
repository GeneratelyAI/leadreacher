// Generated from HarvestAPI's official LinkedIn industry code v2 CSV.
// Source: https://github.com/HarvestAPI/linkedin-industry-codes-v2/blob/main/linkedin_industry_code_v2_all_eng_with_header.csv
// Regenerate from the raw CSV when HarvestAPI updates the actor's industry list.

export type LinkedInIndustryCode = {
  id: number;
  label: string;
  hierarchy: string;
};

export const LINKEDIN_INDUSTRY_CODES = [
  {
    id: 799,
...
] as const satisfies readonly LinkedInIndustryCode[];

const INDUSTRY_ALIASES: Record<string, readonly string[]> = {
  businesstobusinesstechnology: ["Software Development", "IT Services and IT Consulting"],
  businesstobusinesstech: ["Software Development", "IT Services and IT Consulting"],
  businesstobusinesssoftware: ["Software Development"],
  businesstobusinesssoftwareasaservice: ["Software Development"],
  saas: ["Software Development"],
  softwareasaservice: ["Software Development"],
  informationtechnology: ["IT Services and IT Consulting"],
  itservices: ["IT Services and IT Consulting"],
  technologyservices: ["IT Services and IT Consulting"],
  tech: ["Technology, Information and Internet", "Software Development"],
  technology: ["Technology, Information and Internet", "Software Development"],
  dataanalytics: ["Data Infrastructure and Analytics"],
  webdataandanalytics: ["Data Infrastructure and Analytics"],
  // Firecrawl-style web search/crawling platforms are developer software on the internet.
  // This gap surfaced when stricter fuzzy matching correctly rejected an unsafe overlap.
  websearchandcrawling: ["Technology, Information and Internet", "Software Development"],
  dataplatform: ["Data Infrastructure and Analytics"],
  dataplatforms: ["Data Infrastructure and Analytics"],
  dataresearch: ["Data Infrastructure and Analytics"],
  dataandsalesenablement: ["Data Infrastructure and Analytics", "Technology, Information and Internet"],
  datasalesenablement: ["Data Infrastructure and Analytics", "Technology, Information and Internet"],
  businesstobusinessdataandsalesenablement: ["Data Infrastructure and Analytics", "Technology, Information and Internet"],
  businesstobusinessdatasalesenablement: ["Data Infrastructure and Analytics", "Technology, Information and Internet"],
  salesandmarketingtechnology: ["Technology, Information and Internet", "Software Development", "Marketing Services"],
  salestechnology: ["Technology, Information and Internet", "Software Development"],
  marketingtechnology: ["Marketing Services", "Software Development"],
  salesenablement: ["Technology, Information and Internet", "Software Development"],
  salesautomation: ["Software Development", "Technology, Information and Internet"],
  ...
  martech: ["Marketing Services", "Software Development"],
};
...
function tokenizeIndustryName(value: string): string[] {
  return value
    .toLowerCase()
    .replace(/&/g, " and " )
    .replace(/\bb2b\b/g, "business to business")
...
const INDUSTRY_BY_NORMALIZED_NAME = new Map<string, LinkedInIndustryCode>();
const INDUSTRY_BY_LABEL = new Map<string, LinkedInIndustryCode>();
for (const industry of LINKEDIN_INDUSTRY_CODES) {
  INDUSTRY_BY_LABEL.set(industry.label, industry);
  INDUSTRY_BY_NORMALIZED_NAME.set(normalizeIndustryName(industry.label), industry);
  for (const segment of industry.hierarchy.split(">")) {
    const normalized = normalizeIndustryName(segment.trim());
    if (normalized && !INDUSTRY_BY_NORMALIZED_NAME.has(normalized)) {
      INDUSTRY_BY_NORMALIZED_NAME.set(normalized, industry);
    }
  }
}

function scoreIndustryMatch(query: string, industry: LinkedInIndustryCode): number {
  const normalizedQuery = normalizeIndustryName(query);
  const normalizedLabel = normalizeIndustryName(industry.label);
  const normalizedHierarchy = normalizeIndustryName(industry.hierarchy);

  if (normalizedQuery === normalizedLabel) return 1000;
  if (normalizedHierarchy.includes(normalizedQuery)) return 700;
  if (normalizedLabel.includes(normalizedQuery) || normalizedQuery.includes(normalizedLabel)) return 650;

  const queryTokens = tokenizeIndustryName(query);
  if (queryTokens.length === 0) return 0;

  const labelTokens = new Set(tokenizeIndustryName(industry.label));
  const hierarchyTokens = new Set(tokenizeIndustryName(industry.hierarchy));
  const firstQueryToken = queryTokens[0];
  const hasDistinctiveFirstToken =
    firstQueryToken !== undefined &&
    (labelTokens.has(firstQueryToken) || hierarchyTokens.has(firstQueryToken));
  const matchedTokens = queryTokens.filter(
    (token) => labelTokens.has(token) || hierarchyTokens.has(token),
  );

  // Do not turn a partial generic overlap into an industry filter. For example,
  // "Web Data and Analytics" must not match "Climate Data and Analytics".
  if (
    (queryTokens.length > 1 && !hasDistinctiveFirstToken) ||
    matchedTokens.length / queryTokens.length < 0.75
  ) {
    return 0;
  }
  ...
  for (const token of matchedTokens) {
    if (labelTokens.has(token)) score += 120;
    else if (hierarchyTokens.has(token)) score += 70;
  }

  return score / queryTokens.length + (hasDistinctiveFirstToken ? 40 : 0);
}

export function resolveIndustryIds(industries: readonly string[], maxIds = 20): number[] {
  const ids: number[] = [];
  const addId = (id: number) => {
    if (!ids.includes(id) && ids.length < maxIds) {
      ids.push(id);
    }
  };
  ...
  for (const industry of industries) {
    ...
    const aliasLabels = INDUSTRY_ALIASES[normalized];
    if (aliasLabels) {
      for (const label of aliasLabels) {
        const match = INDUSTRY_BY_LABEL.get(label);
        if (match) addId(match.id);
      }
      continue;
    }

    const exactMatch = INDUSTRY_BY_NORMALIZED_NAME.get(normalized);
    if (exactMatch) {
      addId(exactMatch.id);
      continue;
    }

    let bestMatch: LinkedInIndustryCode | null = null;
    let bestScore = 0;
    for (const candidate of LINKEDIN_INDUSTRY_CODES) {
      const score = scoreIndustryMatch(industry, candidate);
      if (score > bestScore) {
        bestMatch = candidate;
        bestScore = score;
      }
