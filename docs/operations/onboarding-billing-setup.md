...
| `STRIPE_PRICE_VIDEO_ADDON` | A live checkout with `videoConfig.enabled=true` | Recurring Stripe Price ID for video personalization. |
...
2. Create the optional recurring Video personalization add-on Price and copy its
   ID to `STRIPE_PRICE_VIDEO_ADDON`.
...
6. Set `STRIPE_MOCK_MODE=false` and deploy the API. A Checkout redirect is not
